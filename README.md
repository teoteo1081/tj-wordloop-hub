...
- **Phiên 2026-09-12 — ĐÃ XONG (lên live, commit `733ea53`):**
  ...
  - "Thanh điều khiển nghe nổi (sticky player)" — giống mini-player "Listen to Page" của Safari (play/pause, thời gian, thanh tiến độ, nút đóng), tự hiện khi cuộn RA KHỎI vùng bài đọc lúc đang nghe (`#passage-content-block` ra khỏi vùng nhìn thấy của `#workspace`), tự ẩn khi cuộn lại vào hoặc hết bài. `js/speech.js` thêm `S.pause()`/`S.resume()`/`S.getProgress()` (Web Speech API hỗ trợ sẵn pause/resume, không cắt utterance như stop() cũ) — tiến độ/tổng thời gian là ƯỚC TÍNH (không có API trình duyệt nào trả về giây thật đã đọc), theo đúng công thức ~14.5 ký tự/giây đã dùng ở `startFallbackTimer`. Chưa test tay trên trình duyệt thật (TJ ngủ, phiên tự làm) — thử lại kỹ trước khi tin 100%: cuộn ra khỏi bài đọc lúc đang nghe có hiện thanh không, bấm ⏸/▶/✕ có đúng không, đặc biệt trên Safari/điện thoại (pause/resume của Web Speech API từng có tiếng là chập chờn tuỳ trình duyệt).
